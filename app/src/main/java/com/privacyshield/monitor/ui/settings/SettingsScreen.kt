    ...
    Scaffold(
        ...
        Column(
            ...
            SectionCard(title = stringResource(R.string.settings_appearance)) {
                Column(verticalArrangement = Arrangement.spacedBy(10.dp)) {
                    ...
                    Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                        Chip(stringResource(R.string.theme_system), settings.themeMode == ThemeMode.SYSTEM) { vm.setTheme(ThemeMode.SYSTEM) }
                        Chip(stringResource(R.string.theme_light), settings.themeMode == ThemeMode.LIGHT) { vm.setTheme(ThemeMode.LIGHT) }
                        Chip(stringResource(R.string.theme_dark), settings.themeMode == ThemeMode.DARK) { vm.setTheme(ThemeMode.DARK) }
                    }
                    Text(stringResource(R.string.settings_language), style = MaterialTheme.typography.labelLarge)
                    Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                        Chip(stringResource(R.string.lang_system), settings.language == AppLanguage.SYSTEM) { vm.setLanguage(context, AppLanguage.SYSTEM) }
                        Chip(stringResource(R.string.lang_arabic), settings.language == AppLanguage.ARABIC) { vm.setLanguage(context, AppLanguage.ARABIC) }
                        Chip(stringResource(R.string.lang_english), settings.language == AppLanguage.ENGLISH) { vm.setLanguage(context, AppLanguage.ENGLISH) }
                    }
                }
            }
                ...
                Column(verticalArrangement = Arrangement.spacedBy(10.dp)) {
                    ...
                    Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                        listOf(7, 30, 90, 365).forEach { d ->
                            Chip("$d", settings.retentionDays == d) { vm.setRetention(d) }
                        }
                    }
            ...
            SectionCard(title = stringResource(R.string.settings_about)) {
                Column {
                    Text(stringResource(R.string.privacy_promise), style = MaterialTheme.typography.bodyMedium)
